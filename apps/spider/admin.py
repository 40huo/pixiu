...
class ProxyAdmin(admin.ModelAdmin):
    list_display = [field.name for field in Proxy._meta.fields if field.name != "id"]


@admin.register(SpiderEvent)
class SpiderEventAdmin(admin.ModelAdmin):
    list_display = [field.name for field in SpiderEvent._meta.fields if field.name != "id"]
